...
cali = 0;%1开校准，0关校准
OSR = 1;
d_len = 16384*OSR; %用于FFT分析的样本长度
K=1.3806505e-23;
T=300;
fin =997;    %fin/d_len为输入频率，fin与d_len互质
...
del_Cu = Cu*0.03;
% del_Cu = 0;
%del_Cu = Cu*0.02e-2;       % 定义工作电容的设计值和分布的均方根值
alfa =0.02; beta =0.02;   % 定义 alfa底板和beta顶板到地的归一化寄生电容。此值是估值，实际取值应从版图提取。底板寄生相对没那么重要，顶板寄生较重要，大概是0.1。正确科学的方法是从版图提取，但是前期算法层面的行为级验证或者SAR ADC逻辑验证时大概定义个0.2就够了。
% alfa =0; beta =0; 
...
%%%%%%%%%%%%%%%%%%%%%%%%%
for i=1:ns
Vip_cal(i,1) = 0.45*Vref*sin(i*delta_ph)+Vcm;% defining input signal 输入单频信号，用sar adc对其进行转换，并将转换结果放到数组adout中
Vin_cal(i,1) = -0.45*Vref*sin(i*delta_ph)+Vcm;
end


weight_cal = cal_dither(Wdit_real,weight,ns,u,ud,Vip_cal,Vin_cal,Vref,Vcm,M,L,Cm_p,...
